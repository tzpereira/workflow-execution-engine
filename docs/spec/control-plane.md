...
- **Delivered by:** M2.2. **Verified by:** `settings.TestSaveLoadRoundTrip`,
  `server.TestSettingsRoundTripSurvivesRestart`, `server.TestReconcileMarksInterruptedRunCancelled`, plus the
  M2.2 kill/restart walkthrough recorded in EXECUTION-PHASE2.md (executions + settings survive; the
  interrupted run reconciles to cancelled).
...
- **Delivered by:** M2.2. **Verified by:** `server.TestReconcileMarksInterruptedRunCancelled` (idempotent),
  `server.TestCancelInFlightRunFinalizesCancelled`, and the M2.2 walkthrough (a hard-killed in-flight run is
  `cancelled`, `running:false` after restart).
...
- **Delivered by:** M2.2. **Verified by:** `server.TestRetryReusesFinishedUpstreamNoDoublePay`,
  `server.TestCancelInFlightRunFinalizesCancelled`, `server.TestCancelNotRunningIs409`,
  `server.TestCacheClearByExecutionNode`, `server.TestBundleDownloadReturnsTar`,
  `engine.TestResumeFromReexecutesNodeAndDownstream`, `cache.TestDeleteRemovesOnlyNamedKeys`,
  `replay.TestExportBundleContainsSnapshotEventsArtifacts`, `ui RunControls.test.tsx`.
...
- **Delivered by:** M2.2. **Verified by:** `server.TestRetryReusesFinishedUpstreamNoDoublePay` (A reused,
  B re-runs once), `engine.TestResumeFromReexecutesNodeAndDownstream`, `engine.TestResumeSkipsFinishedNodes`.
...
- **Delivered by:** M2.2. **Verified by:** `settings.TestSecretValueNeverPersisted`,
  `settings.TestSaveLoadRoundTrip`, `server.TestSettingsRoundTripSurvivesRestart`,
  `ui SettingsModal.test.tsx` (durable-settings save), and the M2.2 walkthrough (settings.json holds only the
  env-var NAME, never a key value).
...
- **Delivered by:** M2.2. **Verified by:** `server.TestDeriveProgress` (counts + running set folded from
  events), `ui RunControls.test.tsx` (progress + liveness readout); the closed catalog is unchanged
  (`domain.TestSchemaDrift`, REQ-EVENT-01).
...
- **Delivered by:** M2.2. **Verified by:** `server.TestEffectiveCacheAndBudgetPrecedence` (request >
  settings > default), `server.TestRunStartsExecutionAndReturnsID` (run params, incl. resolved cache/budget,
  recorded).
...
- **Delivered by:** M2.2. **Verified by:** `settings.TestSaveLeavesNoTempFiles` (temp-then-rename commits
  atomically, no stray temp); the same write path backs run params (`server` writeRunParams).
