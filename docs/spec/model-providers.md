...
- **Delivered by:** M1.4. **Verified by:** _pending_ (vendor-type isolation check: no
  `core/model/{openai,anthropic}` type referenced outside its package).
...
- **Delivered by:** M1.4. **Verified by:** _pending_ (stubbed HTTP server test).
...
- **Delivered by:** M1.4. **Verified by:** _pending_ (base-URL override test against a stub server; manual
  check against a local Ollama is optional, not CI).
