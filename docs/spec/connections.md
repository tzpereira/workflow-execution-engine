...
- **Delivered by:** M2.9. **Verified by:** _pending_ (incl. an import-boundary test asserting no
...
- **Rationale:** PRIN-10 / NFR-SEC-01 — the event log is exportable and hash-chained; a leaked secret would
  be permanent.
- **Delivered by:** M2.9. **Verified by:** _pending_ (grep-of-written-files test in the style of
  `openai.TestNoKeyMaterialInExecutionRecord`).
