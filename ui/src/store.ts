...
function documentFromState(state: WorkspaceState, dirty: boolean): WorkspaceDocument {
  return {
...
function nextNodePosition(nodes: CanvasNode[]) {
  ...
  const columns = 3
  const taken = new Set(nodes.map((n) => `${Math.round(n.position.x / 280)}:${Math.round(n.position.y / 180)}`))
  for (let i = 0; i < nodes.length + 12; i++) {
    const col = i % columns
    const row = Math.floor(i / columns)
    if (!taken.has(`${col}:${row}`)) return { x: 80 + col * 280, y: 80 + row * 180 }
  }
...
export const useWorkspace = create<WorkspaceState>((set, get) => ({
  ...
  importText: (text, format, fileName) => {
    try {
      ...
      set((state) => {
        ...
        return { ...next, ...applyCurrentDocument(next, false) }
      })
    } catch (e) {
  ...
  onNodesChange: (changes) =>
    set((state) => {
      ...
      return { nodes: next.nodes, ...applyCurrentDocument(next) }
    }),
  onEdgesChange: (changes) =>
    set((state) => {
      const next = { ...state, edges: applyEdgeChanges(changes, state.edges) }
      return { edges: next.edges, ...applyCurrentDocument(next) }
    }),
  onConnect: (conn) =>
    set((state) => {
  ...
  addNode: (node, position) => {
    ...
    set((state) => {
      const next = { ...state, nodes: [...state.nodes, canvasNode], selectedNodeId: node.id }
      return { nodes: next.nodes, selectedNodeId: node.id, ...applyCurrentDocument(next) }
    })
  },
  ...
  updateNodeBody: (id, node) =>
    set((state) => {
      const next = {
        ...state,
        nodes: state.nodes.map((n) => (n.id === id ? { ...n, data: { node } } : n)),
      }
      return {
        nodes: next.nodes,
  ...
  setMeta: (meta) =>
    set((state) => {
      ...
      return { meta, ...applyCurrentDocument(next) }
    }),
  newDocument: () =>
    set((state) => {
  ...
  closeDocument: (id) =>
    set((state) => {
      ...
      const active = id === state.activeDocumentId ? docs[0] : state.documents.find((doc) => doc.id === state.activeDocumentId) ?? docs[0]
      return {
        documents: docs,
